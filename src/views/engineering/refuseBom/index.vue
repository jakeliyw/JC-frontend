      ...
      >
        ...
        >
          <template slot-scope="scope">
            <span @click="jumpMateriel(scope.row.fname)" class="jumpMateriel">{{scope.row.fname}}</span>
          </template>
        </el-table-column>
        <el-table-column
          label="状态流程"
        ...
        <template v-slot:btnSlot="clo">
          ...
          <el-button type="warning" size="mini" @click="editBom(clo.scope.row.fname)">修改bom</el-button>
        </template>
      </jc-table>
...
export default {
  ...
  methods: {
    ...
    // 修改bom
    editBom(fname) {
      this.$router.push({ path: `/editBom/${fname}` })
    },
    // 重审bom
    async Retrial(FID) {
