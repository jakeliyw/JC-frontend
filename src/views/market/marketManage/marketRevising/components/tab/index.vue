    ...
    <el-tabs v-model="activeName">
      ...
      <el-tab-pane label="明细信息" name="first">
        ...
        >
          ...
          />
          <el-table-column label="物料描述" prop="fmodel" align="center" min-width="80px" />
...
export default {
  ...
  methods: {
    ...
    async materielSelectRow(item) {
      ...
      this.tabTwo.saleDetails[this.tableIndex].funit = item.funit
...
export default {
  ...
  methods: {
    ...
    // 获取当前行下标
    drawingl(index) {
      console.log(this.tabTwo.saleDetails[index].salImage.imageUrl)
      this.indexSelf = index
      this.isdrawinglDialog = true
    },
    // 关闭上传图片弹窗，并向父组件传值
