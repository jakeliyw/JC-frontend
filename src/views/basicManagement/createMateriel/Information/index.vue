  ...
  <div>
    ...
    <el-row class="elRow" :gutter="30" type="flex" justify="start">
      ...
      <el-col :span="6">
        ...
        <el-input v-model="optionValue.fbillTypeId" placeholder="请选择采购类型" size="mini" class="input-width">
          <i slot="suffix" class="el-input__icon el-icon-search" @click="handlePurchaseType" />
        </el-input>
      </el-col>
      <el-col :span="6">
        <span class="large">配额方式</span>
        <el-select v-model="optionValue.fquotaType" placeholder="请选择配额方式" size="mini" class="input-width" @change="handleFquotaType">
          <el-option
    ...
    <el-row class="elRow" :gutter="30" type="flex" justify="start">
      ...
      <el-col :span="6">
        ...
        <el-input v-model="optionValue.fstockId" placeholder="请选择仓库" size="mini" class="input-width">
          <i slot="suffix" class="el-input__icon el-icon-search" @click="handleWarehouse" />
        </el-input>
      </el-col>
      <el-col :span="6">
        <span class="large">仓位</span>
        <el-input v-model="optionValue.position" placeholder="请输入仓位" size="mini" class="input-width" @change="handlePosition" />
      </el-col>
      <el-col :span="6">
        <span class="large">最小库存</span>
        <el-input v-model="optionValue.fminStock" placeholder="请输入最小库存" size="mini" class="input-width" type="number" @change="handleFminStock" />
      </el-col>
      <el-col :span="6">
        <span class="large">安全库存</span>
        <el-input v-model="optionValue.fsafeStock" placeholder="请输入安全库存" size="mini" class="input-width" type="number" @click="handleFsafeStock" />
      </el-col>
      <el-col :span="6">
        <span class="large">最大库存</span>
        <el-input v-model="optionValue.fmaxStock" placeholder="请输入最大库存" size="mini" class="input-width" type="number" @click="handleFmaxStock" />
      </el-col>
    </el-row>
    <!--  供应商弹窗-->
    <jc-popup
...
export default {
  ...
  methods: {
    ...
    // 仓位
    handlePosition() {
      this.$set(this.information, 'position', this.optionValue.position)
      this.$emit('update:information', this.information)
    },
    // 最小库存
    handleFminStock() {
      this.$set(this.information, 'fminStock', this.optionValue.fminStock)
      this.$emit('update:information', this.information)
    },
    // 安全库存
    handleFsafeStock() {
      this.$set(this.information, 'fsafeStock', this.optionValue.fsafeStock)
      this.$emit('update:information', this.information)
    },
    // 最大库存
    handleFmaxStock() {
      this.$set(this.information, 'fmaxStock', this.optionValue.fmaxStock)
      this.$emit('update:information', this.information)
    },
    // 配额方式
    handleFquotaType() {
      this.$set(this.information, 'fquotaType', this.optionValue.fquotaType)
      this.$emit('update:information', this.information)
    },
    // 配额方式列表
    ...
    supplierSelectRow(item) {
      this.$set(this.information, 'fsupplierId', item.fsupplierId)
      this.$emit('update:information', this.information)
      this.optionValue.fsupplierId = item.fname
      this.openSupplier = false
    },
    // 双击采购弹窗
    purchaseTypeSelectRow(item) {
      this.$set(this.information, 'fbillTypeId', item.fbillTypeId)
      this.$emit('update:information', this.information)
      this.optionValue.fbillTypeId = item.fname
      this.openPurchaseType = false
    },
    // 双击仓库弹窗
    warehouseSelectRow(item) {
      this.$set(this.information, 'fstockId', item.fstockId)
      this.$emit('update:information', this.information)
      this.optionValue.fstockId = item.fname
